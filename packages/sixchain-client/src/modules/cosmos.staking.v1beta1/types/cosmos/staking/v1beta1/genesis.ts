...
import { Delegation, Params, Redelegation, UnbondingDelegation, Validator, ValidatorApproval } from "./staking";
...
export const GenesisState: MessageFns<GenesisState> = {
  encode(message: GenesisState, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.validatorApproval !== undefined) {
      ValidatorApproval.encode(message.validatorApproval, writer.uint32(34).fork()).join();
    }
    for (const v of message.validators) {
      Validator.encode(v!, writer.uint32(42).fork()).join();
    }
    for (const v of message.delegations) {
      Delegation.encode(v!, writer.uint32(50).fork()).join();
    }
    for (const v of message.unbondingDelegations) {
      UnbondingDelegation.encode(v!, writer.uint32(58).fork()).join();
    }
    for (const v of message.redelegations) {
      Redelegation.encode(v!, writer.uint32(66).fork()).join();
    }
