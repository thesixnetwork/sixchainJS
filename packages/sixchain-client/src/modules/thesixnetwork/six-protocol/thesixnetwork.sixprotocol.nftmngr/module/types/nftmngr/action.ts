...
export const ActionParams = {
  encode(message: ActionParams, writer: Writer = Writer.create()): Writer {
    if (message.name !== "") {
  ...
  decode(input: Reader | Uint8Array, length?: number): ActionParams {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseActionParams } as ActionParams;
  ...
  fromPartial(object: DeepPartial<ActionParams>): ActionParams {
    const message = { ...baseActionParams } as ActionParams;
    if (object.name !== undefined && object.name !== null) {
