...
const main = async () => {
  ...
  try {
    ...
    const upgradedConsensusState = await queryClient.cosmos.upgrade.v1beta1.upgradedConsensusState({
    ...
    const moduleVersions = await queryClient.cosmos.upgrade.v1beta1.moduleVersions({
