...
const main = async () => {
  ...
  try {
    ...
    const nftData = await queryClient.sixprotocol.nftmngr.executorOfSchema({
      nftSchemaCode: nftSchemaCode
    });

    console.log(`\n=== NFT Data ===`);
    if (nftData.executorOfSchema) {
      console.log("NFT Schema Code:", nftData.executorOfSchema.nftSchemaCode);
      console.log("Executor:", nftData.executorOfSchema.executorAddress);
    } else {
