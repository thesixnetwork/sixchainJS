...
const main = async () => {
  ...
  console.log(
    `Updating action signer config for chain: ${CHAIN}, contract: ${CONTRACT_NAME} at ${CONTRACT_ADDRESS}`
  );

  // Update action signer config message
  const updateActionSignerConfig =
    sixprotocol.nftoracle.MessageComposer.withTypeUrl.updateActionSignerConfig({
      creator: address,
      chain: CHAIN,
  ...
  const msgArray = [updateActionSignerConfig];

  const memo = "Update NFT Oracle Action Signer Config";
  let txResponse = await signAndBroadcastWithRetry(
    ...
    memo,
    {
      ...
      gasPrice: 1.25,
      fallbackGas:
        COMMON_GAS_LIMITS.NFT_ORACLE.UPDATE_ACTION_SIGNER_CONFIG || 200000,
      denom: "usix",
    }
  );

  if (txResponse.code !== 0) {
    console.error(`Error updating action signer config: ${txResponse.rawLog}`);
    return false;
  } else {
    console.log(
      `Action signer config updated successfully: gasUsed=${txResponse.gasUsed}, gasWanted=${txResponse.gasWanted}, hash=${txResponse.transactionHash}`
    );
    return true;
  }
};
