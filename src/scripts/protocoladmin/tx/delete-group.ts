...
const main = async () => {
  ...
  const msgDeleteGroup =
    sixprotocol.protocoladmin.MessageComposer.withTypeUrl.deleteGroup({
      creator: address,
      name: "token.admin",
    });

  msgArray.push(msgDeleteGroup);

  const memo = "delete new group";
  let txResponse = await signAndBroadcastWithRetry(
    ...
    memo,
    {
      gasMultiplier: 1.5,
      gasPrice: 1.25,
      fallbackGas: COMMON_GAS_LIMITS.PROTOCOMADMIN.DELETE_GROUP,
      denom: "usix",
    }
  );
