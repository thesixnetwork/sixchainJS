...
const main = async () => {
  ...
  const msgAddAdminToGroup =
    sixprotocol.protocoladmin.MessageComposer.withTypeUrl.addAdminToGroup({
      creator: address,
      address: granteeAddress,
      name: "token.admin",
    });

  msgArray.push(msgAddAdminToGroup);

  const memo = "add new admin to group";
  let txResponse = await signAndBroadcastWithRetry(
    ...
    memo,
    {
      gasMultiplier: 1.5,
      gasPrice: 1.25,
      fallbackGas: COMMON_GAS_LIMITS.PROTOCOMADMIN.ADD_ADMIN_TO_GROUP,
      denom: "usix",
    }
  );
