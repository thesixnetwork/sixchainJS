...
const alice_set_actionsigner_request = async () => {
  ...
  const base64_signature = Buffer.from(JSON.stringify(ownerSignature)).toString(
    "base64"
  );
  ...
  const msg = await rpcClient.nftoracleModule.msgCreateActionSigner(
    msgCreateActionSigner
  ); //! suppose to be nftoracleModule will be fixed in next version
